document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        anchor.style.transform = `translate(${moveX/25}px, ${moveY/25}px) scaleX(-1) scale(1.7)`;
    
    });

    applyCardHoverEffects('#left-card', '#middle-card', '60', '#right-card', '60');
    applyCardHoverEffects('#middle-card', '#left-card', '-60', '#right-card', '60');
    applyCardHoverEffects('#right-card', '#middle-card', '-60', '#left-card', '-60');
...
function smoothScroll(){
    document.querySelectorAll('nav a').forEach(link => {
        link.addEventListener('click', function (e) {
            ...
            if (targetElement) {
                window.scrollTo({
                    top: targetElement.offsetTop - document.querySelector('nav').offsetHeight/2, 
                    behavior: 'smooth'
                });
            }
        });
